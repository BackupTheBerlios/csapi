...
\item \vdehome{} is the path to the \verb!vde.sh! executable.
\item \sdshome{} is the installation directory used by the LDAP embedded server.
\end{itemize}

Often, when addressing specific topics, we will simply say \webinf{}/wsconfig since there is only one \webinf{} directory in the \cs{} arborescence. The full path to \webinf{} should be read as \cshome{}/cs\_app/webapps/synergy/\webinf{}.
...
It is composed of several Servlets classes from the Continuus engine (packages com.continuus.*), the \cs{} engine (packages comp.telelogic.*), and some other external classes (e.g. JavaChart). All classfiles are located in \texttt{\cshome{}/cs\_app/webapps/synergy/\webinf{}/classes}. The overall architecture of \cs{} is represented below.
...
The Perl API deserves several goals, from server administration to records reporting. The scripts provided in this chapter have proven to be useful to integrate the \cs{} setup smoothly in company environments.

Such are especially useful when used in cron tabs. They allow a delayed check of consistency in the database, update of computed values, batch treatment to export data toward other tools, etc.

\subsection{Get a report}

This script retrieves attributes for either a query or a set of problem\_number's. The list of attributes to be retrieved must be passed on command line, and formatting of the ouput can be obtained by specifying the -f switch (using the classic C syntax, cf. man fprintf).

\subsection{Modify records}

This scripts allow someone to modify a record on the \cs{} server, according to the rights of the user provided for login.
...
\item modify the attribute and set the original value.
\end{enumerate}
...
It would be quite straightforward to remove an item, but difficult enough to add a newly created one. Templates can easily be modified to hide or show some parts of the interface, such as one of the top buttons as explained in \S \ref{sec:custUI}. Customization for a particular \csrole{}\footnote{Remember that different \csrole{} can (and usually) have different web interfaces.} has two steps:
