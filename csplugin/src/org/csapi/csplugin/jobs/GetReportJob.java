...
/**
 * <p>
 * The Job class for the Get Report command in the global menu.
 * </p>
 * 
 * <p>
 * It prompts for query and attributes (in the current thread workspace, then
 * get the default sessionMgr and execute the query. When result is returned,
 * the view's viewer is updated.
 * </p>
 * 
...
public class GetReportJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		final String query = (String) JOptionPane.showInputDialog(myJFrame,
				"Please provide query:", "Query", JOptionPane.OK_CANCEL_OPTION,
				null, null, "(product_name='Product A')");
		// if cancel is hit
		if (query == null) {
			return new Status(Status.CANCEL, "CSPlugin", 0, "Query cancelled",
					null);
		}

		// Get the list of attributes.
		final String attributesList = (String) JOptionPane.showInputDialog(
				myJFrame,
				"Please provide attributes list, separated by pipes:",
				"Attributes needed", JOptionPane.OK_CANCEL_OPTION, null, null,
				"problem_number|problem_synopsis");
		// if cancel is hit
			...
			public void run() {
				try {
					...
					if (inst != null && report != null) {
						((ShowReportView) inst).setColumns(report
								.getAttributes());
		...
		return new Status(Status.OK, "CSPlugin", 0, "Get Report successfull.",
				null);
	}
